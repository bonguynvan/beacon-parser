...
- `packages/mcp/` — the published `beacon-mcp` MCP server (stdio, local only): `parse_hit`, `explain_hit`, `diff_hits`, `validate_hits`. Depends on beacon-parser, tracking-plan and `@modelcontextprotocol/sdk` (+ `zod`). `validate_hits` executes a local plan file, so `plan-loader.ts` confines it to a root dir (symlinks resolved) — do not loosen that. `explain.ts` holds field meanings + Adobe source URLs: paraphrase only, cite a source per claim, and add a caveat instead of guessing (same rules as `docs/parameters.md`).
