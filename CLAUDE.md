...
- `packages/cli/` — the published `beacon-qa` CLI (Phase 4), also a small library (`loadConfig`, `runFlows` in `src/index.ts`; `runFlows` has a `blockHits` option that aborts Adobe requests after they're captured); depends on beacon-parser, beacon-playwright, tracking-plan, and `playwright` itself (the only package with a real, non-workspace runtime dependency)
- `packages/mcp/` — the published `beacon-mcp` MCP server (stdio, local only): `parse_hit`, `explain_hit`, `diff_hits`, `validate_hits`, `run_flow`. Depends on beacon-parser, tracking-plan, beacon-cli (imported lazily, only by `run_flow`) and `@modelcontextprotocol/sdk` (+ `zod`). `validate_hits` and `run_flow` execute local files (plans / flow configs), so `resolveInsideRoot()` in `plan-loader.ts` confines both to a root dir with symlinks resolved — do not loosen that, and never let a tool accept URLs or code from the agent. `run_flow` blocks Adobe hits after capture unless `sendHits` is set, so agent-driven runs don't pollute a real report suite. `explain.ts` holds field meanings + Adobe source URLs: paraphrase only, cite a source per claim, and add a caveat instead of guessing (same rules as `docs/parameters.md`).
...
- `skills/adobe-analytics-hits/SKILL.md` — drop-in assistant skill: rules + a condensed, sourced parameter table + common mistakes. It duplicates part of `docs/parameters.md` on purpose (it must stand alone in a user's project), so **when a row in `docs/parameters.md` changes, update the matching row in SKILL.md and `lab/site/llms.txt`** in the same commit. Same rules: paraphrase Adobe, cite a source per claim, mark unknowns `TODO: verify`; anything the wire can't prove (e.g. a `linkTrackVars` cause) is labeled an inference, and non-Adobe field experience is labeled as such.
